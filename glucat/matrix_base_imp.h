...
namespace glucat
{
  namespace matrix
  {
    ...
    inline const Derived_T& matrix_base<Derived_T>::derived() const
     ...
     */
    template <typename Derived_T>
    inline Derived_T& matrix_base<Derived_T>::derived()
     ...
     */
    template <typename Derived_T>
    inline eig_genus<Derived_T> matrix_base<Derived_T>::classify_eigenvalues() const
    {
      ...
      static const auto epsilon = std::max(std::numeric_limits<double>::epsilon(),
      ...
      for (auto k = decltype(dim)(0); k != dim; ++k)
      {
        ...
        if (!neg_real_eig_found && real_lambda_k < -epsilon && (imag_lambda_k == 0.0 || imag_lambda_k * imag_lambda_k < norm_tol))
          neg_real_eig_found = true;
        if (!imag_eig_found && imag_lambda_k > epsilon && (real_lambda_k == 0.0 || real_lambda_k * real_lambda_k < norm_tol))
          imag_eig_found = true;
        if (!zero_eig_found && std::norm(lambda_k) < zero_eig_tol)
          zero_eig_found = true;
      }
      ...
      if (result.m_eig_case == both_eigs)
      {
        ...
        for (++arg_it; arg_it != arg_set.end(); ++arg_it)
        {
