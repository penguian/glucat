...
the intermediate output files ./test00/test00.out to ./test17/test17.out, and the
...
order. With zero parameters, all examples from 00 to 17 are run in order. Many
of the examples are run twice - once with framed_multi<Scalar_T> and once with
matrix_multi<Scalar_T>.

The ./test_runtime directory contains 30 sample versions of the regression test
results, corresponding to 15 different combinations of configuration parameters,
...
./configure
  ...
  2. test.configure.compressed-mat.out:
./configure --without-dense-mat

  3. test.configure.debug-full.out:
./configure --enable-debug=full

  4. test.configure.debug-yes.out:
./configure --enable-debug=yes

  5. test.configure.disable-check-isnan.out:
./configure --disable-check-isnan

  6. test.configure.disable-dependency.out:
./configure --disable-dependency-tracking

  7. test.configure.disable-pyclical.out:
./configure --disable-pyclical

  8. test.configure.map-map-gsl.out:
./configure --with-map=map --with-random=gsl

  9. test.configure.map-map-pool-alloc.out:
./configure --with-map=map --with-pool-alloc

 10. test.configure.prefix-home-opt.out:
./configure --prefix=$HOME/opt

 11. test.configure.eig-bindings.out:
./configure --with-eig=bindings --with-extra-includes=$PATHTO/numeric_bindings

 12. test.configure.eig-bindings-libcxx.out:
./configure --with-eig=bindings --with-extra-includes=$PATHTO/numeric_bindings \
            --disable-pyclical --with-stdlib=libcxx

 13. test.configure.eig-bindings-qd.out:
./configure --with-eig=bindings --with-extra-includes=$PATHTO/numeric_bindings \
            --with-qd

 14. test.configure.eig-bindings-qd-gsl.out:
./configure --with-eig=bindings --with-extra-includes=$PATHTO/numeric_bindings \
            --with-qd --with-random=gsl

 15. test.configure.eig-bindings-qd-map-map.out:
./configure --with-eig=bindings --with-extra-includes=$PATHTO/numeric_bindings \
            --with-qd --with-map=map

For each of the 15 test.configure.*.out files, there is a corresponding
fast-test.configure.*.out file, making a total of 30 files.
...
The reason why sample test results corresponding to 15 different combinations
...
o If --with-eig=X is chosen, with X set to bindings, the algorithms used for the
  ...
  accurate, and the tests in ./test11/test11 will succeed.
  Even if this option is chosen, some tests in ./test11/test11 fail due to
  insufficient accuracy. This is most likely caused by a combination of
...
options specified in the file. The syntax of each line of
...
line 8 of ./test/config-options.txt
...
output of ./test/fast-test-all-config-options.sh just examine all of the
...
to use sudo, login as root, or su to root before you run make install.
 ...
 1) Pensieri:
    ...
    All 15 configuration commands corresponding to each of the 15
