"""Tutorial 1.1 Complex Numbers for PyClical."""
# -*- coding: utf-8 -*-
#
...
def run(ctx):
    """Run tutorial 1.1 Complex Numbers."""
    ...
    print_line()
    print_fill(
        "Clifford algebras contain many square roots of -1. "
        + " The simplest such algebra is generated by {-1} and is isomorphic to"
        + " the complex numbers. "
        + " In this tutorial, we therefore identify i, the square root of -1"
        + " in the complex numbers, with the generator {-1}."
    )
    print_line()
    ...
    print_line()
    print_fill(
        "Example 9.1. Compute the absolute value r, the square root s, and the"
        + " cube t of the complex number z = 3 + 4*i."
    )
    print_line()
    ...
    print_line()
    print_fill(
        "Example 9.2. Compute log(1+i) and compare the result to the principal"
        + " logarithm (log(2))/2 + i*pi/4."
    )
    print_line()
    ...
    pause()
    check_eval(
        "compare with z to check that the value of w is correct.",
        "exp(w)",
        "print(abs(z - {}))",
    )
    ...
    pause()
    check_eval(
        "compare with 1+i to check that z == 2*k*pi + w is a solution.",
        "sin(2*k*pi + w)",
        "print(abs((1+i) - {}))",
    )
    pause()
    check_eval(
        "compare with 1+i to check that z == (2*k+1)*pi - w is a solution.",
        "sin((2*k+1)*pi - w)",
        "print(abs((1+i) - {}))",
    )
    ...
    print_line()
    print_fill(
        "You have completed the tutorial file pyclical_tutorial_1_1_complex.py."
    )
