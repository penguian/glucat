...
def run(ctx):
    ...
    check_exec(
        "set gamma to be the coefficient of r with respect to c.",
        "gamma",
        "(a ^ b ^ r) / V",
    )
    check_eval(
        "compare with r to check that the values of alpha, beta and gamma are correct.",
    ...
    pause()
    check_exec(
        "set r to be the component of v perpendicular to A.", "r", "(v ^ A) / A"
    )
    check_eval(
        "compare with v to check that the values of p and r are correct.",
        "p + r",
        "print(abs(v - ({})))",
    )
    ...
    pause()
    check_exec(
        "set t3 to be the unit vector parallel to u3.", "t3", "u3 / abs(u3)"
    )

    pause()
    print_line()
    print_fill(
        "Now, check that the vectors t1, t2 and t3 are mutually orthogonal."
    )
    print_line()
    ...
    pause()
    check_eval(
        "check that t2 is orthogonal to t3", "t2 * t3 + t3 * t2", "print({})"
    )

    pause()
    print_line()
    print_fill(
        "Note that the process used to obtain t1, t2 and t3 is essentially"
    ...
    print_line()
    print_fill(
        "Note that we have used -a instead of a here, to allow the use of r | s. "
    ...
    print_line()
    print_fill(
        "Because abs(a) == 2.5 < pi, we can represent the rotation uniquely by -a,"
    ...
    pause()
    check_exec(
        "set q2 to be the vector r rotated by the rotor s2.", "q2", "r | s2"
    )
    ...
    print_line()
    print_fill(
        "This means that the axis of the combined rotation is OC, C == (0,0,1)."
    )
    print_line()
    ...
    print_line()
    print_fill(
        "You have completed the tutorial file pyclical_tutorial_1_2_space.py."
    )
