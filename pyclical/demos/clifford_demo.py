...
def run(ctx):
    ...
    print_line()
    print_fill(
        r"The *scalar product* of $a$ and $b$ is denoted by $a \cdot b$ (or $(a,b)$ or $\langle a,b \rangle$)."
    )
    print_fill(
        r"If we express $b$ as $b=b_{||}+b_{\perp}$, where $b_{||}$ is parallel to $a$, and"
    ...
    print_fill(
        r"$v_1 \wedge v_2 \wedge v_3$ is a *trivector* - a directed volume of a 3-space."
    )
    print_fill(
        r"$v_1 \wedge v_2 \wedge \ldots \wedge v_n$ is an $n$-*vector* - a directed volume of an $n$-space."
    )
    print_line()
    ...
    print_fill(
        r"The set of vectors $\{v_1, v_2, \ldots, v_m\}$ is linearly dependent if and only if"
    ...
    print_fill(
        r"It detects when two vectors $v_1, v_2$ are orthogonal."
    ...
    print_fill(
        r"$e_1 e_2 = e_1 \wedge e_2, \ldots, e_1 e_2 \ldots e_n = e_1 \wedge e_2 \wedge \ldots \wedge e_n$"
    )
    print_exec("print(e(1)*e(2)); print(e(1) ^ e(2))")
    print_fill(
        r"... so that every multivector $x$ in the $2^n$-dimensional Grassmann algebra on $\mathbb{R}^n$ can be expressed as"
    )
    print_fill(
    ...
    print_fill(
        r"Well, now that we know that the Clifford square of every vector is the square of its length,"
    ...
    print_fill(
        r"The expression $-a b / a = -a b a^{-1}$ is the reflection of $b$ in the direction of $a$."
    )
    print_fill(
        r"$-a b a^{-1} = -a (b_{\perp} + b_{||}) a^{-1} = -a b_{\perp} a^{-1} -a b_{||} a^{-1}$"
    ...
    pause()
    print_fill(
        r"The *Cartan-Dieudonne' Theorem* says that every orthogonal transformation on $\mathbb{R}^n$"
        + r" can be expressed as the product of at most $n$ vectors."
    )
    print_fill(
        r"In particular, $x \mapsto b a x (b a)^{-1}$ is a reflection in the direction of $a$"
    ...
    print_fill(
        r"$x \mapsto \operatorname{e}^{B} x \operatorname{e}^{-B} = (-\operatorname{e}^{B}) x (-\operatorname{e}^{-B})$."
    )
    ...
    print_fill(
        "In this space, the special orthogonal transformations connected to the identity"
