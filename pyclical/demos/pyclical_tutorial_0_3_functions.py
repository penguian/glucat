...
def run(ctx):
    ...
    print_line()
    print_fill(
        "These can be called either as a member function or as an ordinary function."
    )
    print_line()
    ...
    print_line()
    print_fill(
        "The outer product power is only defined for m as a non-negative integer."
    )
    print_line()
    ...
    print_line()
    check_exec("set y to be the inverse of w.", "y", "inv(w)")
    check_exec("set z to be the cube of x.", "z", "pow(x, 3)")
    check_exec("set y to be w to the power of w.", "y", "pow(w, w)")
    check_exec("set z to be the outer cube of x.", "z", "outer_pow(x, 3)")
    ...
    print_line()
    print_fill(
        "Most of these can be called as member functions or as ordinary functions."
    )
    print_line()
    print_fill(
        "One exception is for the grade-vector part of a clifford object. "
    ...
    check_exec("set y to be the grade 3 part of w.", "y", "w(3)")
    ...
    print_line()
    print_fill(
        "real(): Synonym for scalar().  This is an ordinary function only."
    )
    print_line()
    print_exec("print(scalar(x))")
    ...
    print_line()
    print_fill(
        "pure(): Pure part of a clifford object, that is a clifford object minus"
        + " its scalar part."
    )
    print_line()
    ...
    print_line()
    print_fill(
        "vector_part(frm): Vector part of clifford object, as a Python list."
    )
    print_line()
    print_fill(
        "This is only available as a member function. "
    ...
    print_fill("Try these now.")
    print_line()
    check_exec("set y to be the scalar part of w.", "y", "scalar(w)")
    check_exec("set z to be the pure part of w.", "z", "pure(w)")
    check_eval(
        "compare with w to check that the scalar and pure parts add to w.",
        "scalar(w) + pure(w)",
        "print(abs( w - ({}) ))",
    )
    check_exec("set y to be the even part of w.", "y", "w.even()")
    check_exec("set z to be the odd part of w.", "z", "w.odd()")
    check_eval(
        "compare with w to check that the even and odd parts add to w.",
        "even(w) + odd(w)",
        "print(abs( w - ({}) ))",
    )
    check_exec(
        "set y to be the vector part of w with respect to the frame {-2,1,3}",
        "y",
    ...
    print_line()
    print_fill(
        "Pyclical implements the three Clifford algebra involutions, both as"
        + " member functions, and as ordinary functions:"
    )
    print_line()
    print_fill(
        "involute(): Grade involution, each {i} is replaced by -{i} in each term,"
        + " eg. e(1) -> -e(1)."
    )
    print_line()
    ...
    print_line()
    print_fill(
        "reverse(): Reversion anti-automorphism, eg. e(1)*e(2) -> e(2)*e(1)."
    )
    print_line()
    ...
    print_fill("Try these now.")
    print_line()
    check_exec("set y to be the grade involute of w.", "y", "involute(w)")
    check_exec("set z to be the reverse of w.", "z", "reverse(w)")
    check_exec("set y to be the Clifford conjugate of x.", "y", "conj(x)")
    ...
    print_line()
    print_fill(
        "max_abs(): Maximum of absolute values of components of the clifford object."
    )
    print_line()
    ...
    print_fill("Try these now.")
    print_line()
    check_exec("set y to be the norm of w.", "y", "norm(w)")
    check_exec("set z to be the absolute value of w.", "z", "abs(w)")
    check_exec(
        "set y to be the maximum absolute value of the coordinates of x.",
        "y",
        "max_abs(x)",
    )
    check_exec("set z to be the quadratic form of w.", "z", "quad(w)")

    pause()
    print_line()
    print_fill(
        "Two more properties of a clifford object are available as member functions:"
    )
    print_line()
    print_fill(
        "frame(): Index set defining a subalgebra that contains the clifford object."
    )
    print_line()
    ...
    print_line()
    print_fill(
        "isnan(): Check if the clifford object contains any IEEE NaN values."
    )
    print_line()
    ...
    print_line()
    print_fill(
        "The following member function simplifies the value of a clifford object. "
        + " This is especially useful when printing."
    )
    print_line()
    print_fill(
        "truncated(limit): Remove all terms with relative size smaller than limit."
    )
    print_line()
    ...
    print_line()
    print_fill(
        "You have completed the tutorial file pyclical_tutorial_0_3_functions.py."
    )
