...
from pyclical_tutorial_utils import get_object_methods, tutorial_context
...
def run(ctx):
    ...
    print_line()
    print_fill(r"The *scalar product* of $a$ and $b$ is denoted by $a \cdot b$ (or $(a,b)$ or $\langle a,b \rangle$).")
    print_fill(r"If we express $b$ as $b=b_{||}+b_{\perp}$, where $b_{||}$ is parallel to $a$, and"
              +r" $b_{\perp}$ is orthogonal to $a$, then we have")
    print_fill(r"$|a \cdot b| = \| a \| \| b_{||} \|$, and $a \cdot b = b \cdot a$.")
    print_exec("print(a & b); print(b & a)")

    pause()
    print_fill("Grassmann's *exterior (wedge) product* obeys different rules ...")
    print_fill(r"$|a \wedge b| = \|a\| \|b_{\perp}\|$, and $a \wedge b = - b \wedge a$.")
    print_exec("print(a ^ b); print(b ^ a)")
    ...
    print_fill(r"$v_1 \wedge v_2$ is a *bivector* - a directed area in a 2-plane.")
    print_fill(r"$v_1 \wedge v_2 \wedge v_3$ is a *trivector* - a directed volume of a 3-space.")
    print_fill(r"$v_1 \wedge v_2 \wedge \ldots \wedge v_n$ is an $n$-*vector* - a directed volume of an $n$-space.")
    print_line()
    print_fill(r"There is no $n+1$-vector in $n$-space. Why?")

    pause()
    print_fill("The wedge product detects linear dependence.")
    print_fill(r"The set of vectors $\{v_1, v_2, \ldots, v_m\}$ is linearly dependent if and only if"
              +r" the product $v_1 \wedge v_2 \wedge \ldots \wedge v_m = 0$.")
    print_exec("print(a)")
    print_exec("print(b)")
    ...
    print_fill(r"$a b := a \wedge b + a \cdot b$.")
    print_exec("c = a * b; print(c)")

    pause()
    print_fill("The Clifford product has remarkable properties.")
    print_fill(r"It detects when two vectors $v_1, v_2$ are orthogonal."
              +r" In this case, they *anticommute*: $v_1 v_2 = - v_2 v_1$.")
    ...
    print_fill(r"$b b = b \wedge b + b \cdot b = b \cdot b = \|b\|^2$.")
    print_exec("b = 3*e(1)+4*e(2); print(b); print(b*b); print(abs(b)**2)")

    pause()
    print_fill(r"If $\{e_1, e_2, \ldots, e_n\}$ is an orthogonal basis of $\mathbb{R}^n$,"
    ...
    print_fill(r"Well, now that we know that the Clifford square of every vector is the square of its length,"
    ...
    pause()
    print_fill(r"With the convention that $a/b := a b^{-1}$, we can now divide vectors.")
    ...
    print_fill(r"The expression $-a b / a = -a b a^{-1}$ is the reflection of $b$ in the direction of $a$.")
    print_fill(r"$-a b a^{-1} = -a (b_{\perp} + b_{||}) a^{-1} = -a b_{\perp} a^{-1} -a b_{||} a^{-1}$"
              +r"$= a a^{-1} b_{\perp} - a a^{-1} b_{||} = b_{\perp} - b_{||}$.")
    print_exec("print(a)")
    print_exec("print(b); print(abs(b))")
    print_exec("c = -a * b / a; print(c); print(abs(c))")

    pause()
    print_fill(r"The *Cartan-Dieudonne' Theorem* says that every orthogonal transformation on $\mathbb{R}^n$"
              +r" can be expressed as the product of at most $n$ vectors.")
    print_fill(r"In particular, $x \mapsto b a x (b a)^{-1}$ is a reflection in the direction of $a$"
              +r" followed by a reflection in the direction of $b$.")
    ...
    pause()
    print_fill(r"It turns out that every special orthogonal transformation can be expressed as"
              +r" the exponential of a bivector (when $n \geq 2$) ...")
    print_fill(r"$x \mapsto \operatorname{e}^{B} x \operatorname{e}^{-B} = (-\operatorname{e}^{B}) x (-\operatorname{e}^{-B})$.")
    print_exec("print(x); print(x*x)")
    print_exec("y = exp(a ^ b) * x * exp(-a ^ b); print(y); print(y*y)")
    print_fill(r"... and the set of exponentials of bivectors forms a group called the"
              +r" *Spin group* $\operatorname{Spin}(n)$ over $\mathbb{R}^n$.")

    pause()
    print_fill("But whatever happened to *time* ...?")
    print_fill(r"Einstein and Minkowski showed that *space-time* can be described using"
              +r" the *Minkowski space* $\mathbb{R}^{3,1}$.")
    print_fill(r"This is a space with a basis $\{e_{\{-1\}},e_{\{1\}},e_{\{2\}},e_{\{3\}}\}$,"
              +r" where $e_{\{-1\}}^2 = -1$.")
