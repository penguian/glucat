...
For more on installing from Git, see INSTALL.
...
./doc, ./gfft_test, ./glucat, ./m4, ./products, ./pyclical, ./squaring, ./test,
...
To run clifford_demo.py, pyclical_demo.p, or sqrt_log_demo.py, use the
following commands:

% cd /path/to/my/demos
% python3 $DEMO.py
where $DEMO is one of clifford_dmo, pyclical_demo or sqrt_log_demo.
...
the directories /usr/local/doc/glucat/api/html and /usr/local/doc/glucat/api/pdf.
...
The clifford_algebra::truncated() member function sets relatively small terms
of the multivector *this to zero. The function compares the absolute value of
each term to the largest absolute value of any term, and sets to zero any term
whose relative absolute value is smaller than the given limit. The default limit
is std::numeric_limits<Scalar_T>::epsilon(). The truncated() matrix function
operates similarly to set relatively small matrix entries to zero.
...
Makefile. Your Makefile needs to pass the appropriate flags to your compiler.
 ...
 o Make sure that the header installation directory is in your include path.
