...
namespace glucat
{
  ...
  // clifford_algebra operations
    ...
    virtual const bool          operator==  (const multivector_t& val) const = 0;
    /// Test for equality of multivector and scalar
    virtual const bool          operator==  (const Scalar_T& scr) const =0;
    /// Geometric sum
    ...
    virtual const Scalar_T      operator[]  (const index_set_t& ist) const =0;
    ...
    virtual const Scalar_T      norm()      const =0;
    /// Scalar_T quadratic form == (rev(x)*x)(0)
    virtual const Scalar_T      quad()      const =0;
    virtual const Scalar_T      max_abs()   const =0;
    virtual const multivector_t truncated
                  (const Scalar_T& limit = Scalar_T(DEFAULT_TRUNCATION)) const =0;
    /// Check if a multivector contains any IEEE NaN values
    virtual const bool          isnan()     const =0;
...
#define _GLUCAT_CLIFFORD_ALGEBRA_OPERATIONS \
    const bool          operator==  (const multivector_t& val) const; \
    const bool          operator==  (const Scalar_T& scr) const;      \
    ...
    const Scalar_T      operator[]  (const index_set_t& ist) const;   \
    ...
    const Scalar_T      quad()      const;                            \
    const index_set_t   frame()     const;                            \
    const Scalar_T      max_abs()   const;                            \
