...
"""
    ...
    print_line()
    print_fill(
        "Example 7.1. Divide the vector r == 5{1}-{2} into components parallel to"
    ...
    pause()
    check_exec(
        "set beta to be the coefficient of r with respect to b.",
        "beta",
        "(a ^ r) / (a ^ b)",
    )
    pause()
    check_eval(
        "compare with r to check that the values\n"
    ...
    print_line()
    print_fill(
        " In this example a ^ b is the oriented area of the parallelogram with"
        + "  sides a and b."
    )
    print_line()
    ...
    print_line()
    print_fill(
        "Example 7.2. Compute the angle between the directions a == 3{1}+{2} and"
        + " b == 4{1}-2{2}."
    )
    print_line()
    ...
    pause()
    check_exec(
        "set theta to be the angle between a and b,"
    ...
    pause()
    check_eval(
        "compare with scalar(a & b) to check that the value of theta is correct.",
    ...
    print_line()
    print_fill(
        "The value of theta is expressed in radians and corresponds to 45 degrees. "
    ...
    print_line()
    print_fill(
        "Example 8.1. Find the projection of the vector a == 8{1}-{2} in the"
    ...
    pause()
    check_exec(
        "set c to be the projection of a in the direction b.",
        "c",
        "(a & b) / b",
    )
    pause()
    check_exec(
        "set d to be the component of a perpendicular to b.", "d", "(a ^ b) / b"
    )
    pause()
    check_eval(
        "compare with a to check that the values of c and d are correct.",
        "c + d",
        "print(abs(a - (c + d)))",
    )
    ...
    print_line()
    print_fill(
        "Example 8.2. Rotate the vector r==2{1}+3{2} by the angle pi/3==60 degrees."
    )
    print_line()
    ...
    pause()
    check_exec(
        "set rz to be the vector r rotated by the angle pi/3.", "rz", "r * z"
    )
    pause()
    check_exec(
        "set zr to be the vector r rotated by the angle -pi/3.", "zr", "z * r"
    )

    pause()
    print_line()
    print_fill(
        "Note that the rotations r |-> r*z and r |-> z*r have opposite orientations. "
    ...
    print_line()
    print_fill(
        "Example 8.3. Combine two successive reflections of the vector r==4{1}-3{2}"
    ...
    pause()
    check_exec(
        "set q to be the vector r reflected in the line defined by the vector a.",
        "q",
        "r | a",
    )
    pause()
    check_exec(
        "set p to be the vector q reflected in the line defined by the vector b.",
        "p",
        "q | b",
    )
    ...
    print_line()
    print_fill(
        "The notation r | a means a * r / involute(a), and if a is a vector,"
    ...
    pause()
    check_eval(
        "compare with r | a to check the meaning of r | a.",
        "a * r / involute(a)",
        "print(abs( (r | a) - ({}) ))",
    )
    pause()
    check_eval(
        "compare with r | a to check the first equality above.",
        "-a * r / a",
        "print(abs( (r | a) - ({}) ))",
    )
    pause()
    check_eval(
        "compare with r | a to check the second equality above.",
        "(r * a - 2 * (a & r)) / a",
        "print(abs( (r | a) - ({}) ))",
    )
    pause()
    ...
    print_line()
    print_fill(
        "You have completed the tutorial file pyclical_tutorial_1_0_plane.py."
    )
