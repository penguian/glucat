...
namespace glucat
{
  namespace gen
  {
    ...
    auto generator_table<Matrix_T>::generator() -> generator_table<Matrix_T>&
    {
     ...
     */
    template <class Matrix_T>
    inline auto generator_table<Matrix_T>::operator()(const index_t p, const index_t q) -> const Matrix_T*
    {
     ...
     */
    template <class Matrix_T>
    auto generator_table<Matrix_T>::gen_vector(const index_t p, const index_t q) -> const std::vector<Matrix_T>&
    {
     ...
     */
    template <class Matrix_T>
    void generator_table<Matrix_T>::gen_from_pm1_qm1(const std::vector<Matrix_T>& old, const signature_t sig)
    {
      ...
      for (auto k = size_t(1); k != new_size - 1; ++k)
     ...
     */
    template <class Matrix_T>
    void generator_table<Matrix_T>::gen_from_pm4_qp4(const std::vector<Matrix_T>& old, const signature_t sig)
    {
      ...
      auto h = old[0];
      for (auto k = size_t(1); k != size_t(4); ++k)
        h = old[k] * h;

      for (auto k = size_t(0); k != old_size - 4; ++k)
        result[k] = old[k + 4];
      for (auto k = old_size - 4; k != old_size; ++k)
     ...
     */
    template <class Matrix_T>
    void generator_table<Matrix_T>::gen_from_pp4_qm4(const std::vector<Matrix_T>& old, const signature_t sig)
    {
      const auto old_size = old.size();
      using size_t = decltype(old_size);
      using result_t = std::vector<Matrix_T>;
      ...
      for (auto k = size_t(0); k != size_t(4); ++k)
        result[k] = old[k + old_size - 4] * h;
      for (auto k = size_t(4); k != old_size; ++k)
        result[k] = old[k - 4];
      // Save the resulting generator array.
      this->insert(make_pair(sig, result));
    }
     ...
     */
    template <class Matrix_T>
    void generator_table<Matrix_T>::gen_from_qp1_pm1(const std::vector<Matrix_T>& old, const signature_t sig)
    {
      const auto old_size = old.size();
      using size_t = decltype(old_size);
      using result_t = std::vector<Matrix_T>;
      ...
      for (auto k = size_t(0); k != old_size - 1; ++k)
...
TEST_CASE("gen::generator_table")
{
  ...
  SUBCASE("Generation Edge Cases")
  {
    ...
    wide_index_set_t p8;
    for (int i = 1; i <= 8; ++i)
      p8 |= wide_index_set_t(i);
    wide_index_set_t n8;
    for (int i = 1; i <= 8; ++i)
      n8 |= wide_index_set_t(-i);
