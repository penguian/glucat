...
def run(ctx):
    ...
    print_line()
    check_exec("set y to be the unary operator + applied to w.", "y", "+w")
    check_exec(
        "set z to be the unary operator - applied to x, that is, the negative of x.",
        "z",
        "-x",
    )
    ...
    print_line()
    print_fill(
        "The binary comparison operators == and != are defined for clifford objects,"
    ...
    print_fill("Try these now.")
    print_line()
    check_eval("check that x * 1.0 equals x.", "(x * 1.0) == x", "print({})")
    check_eval("check that w does not equal x.", "w != x", "print({})")
    check_eval("check that 2 does not equal x.", "2 != x", "print({})")
    ...
    print_line()
    print_fill(
        "The binary operators + and - are defined for clifford objects and are also"
    ...
    print_fill("Try these now.")
    print_line()
    check_exec("set y to be the sum of w and x.", "y", "w + x")
    check_exec(
        "set z to be the difference between w and x, that is, w minus x.",
        "z",
        "w - x",
    )
    check_exec("set y to be the sum of w and 2.5.", "y", "w + 2.5")
    check_exec(
        "set z to be the difference between 3.5 and x, that is, 3.5 minus x.",
        "z",
        "3.5 - x",
    )
    ...
    print_line()
    print_fill(
        "Objects of type clifford can be modified by using assignment addition and"
    ...
    print_line()
    print_fill(
        "Using assignment subtraction to set z to be the difference between z and x:"
    )
    print_line()
    ...
    print_line()
    print_fill(
        "Unlike Python floating point numbers, there are 4 different multiplication"
        + " operators defined for clifford objects, which also work between clifford"
        + " objects and floating point numbers."
    )
    print_line()
    print_fill(
        "These are: * Clifford or geometric product; also scalar multiplication."
    )
    ...
    print_fill("Try these now.")
    print_line()
    check_exec("set y to be the Clifford product of w and x.", "y", "w * x")
    check_exec("set z to be the wedge product of w and x.", "z", "w ^ x")
    check_exec(
        "set y to be the Hestenes inner product of w and x.", "y", "w & x"
    )
    check_exec(
        "set z to be the left contraction between w and x.", "z", "w % x"
    )

    pause()
    print_line()
    print_fill(
        "Note that the precedence for these operators is the same as in Python and"
    ...
    print_line()
    print_fill(
        "Recall that the Clifford product * is not commutative. "
    ...
    print_line()
    check_exec("set y to be x divided by w.", "y", "x / w")

    pause()
    print_line()
    print_fill(
        "The binary operation x | y, where x and y are clifford object,"
    ...
    print_line()
    check_exec(
        "set y to be x transformed by the twisted adjoint action of w.",
        "y",
        "x | w",
    )
    ...
    print_line()
    print_fill(
        "Exponentiation is expressed as usual in Python using the operation x ** y. "
    ...
    pause()
    check_exec("set y to be the cube of x.", "y", "x ** 3")
    check_exec("set z to be 3 to the power of x.", "z", "3 ** x")
    check_exec("set y to be w to the power of x.", "y", "w ** x")

    pause()
    print_line()
    print_fill(
        "You have completed the tutorial file pyclical_tutorial_0_2_operations.py."
    )
