INSTALL for GluCat 0.98a3 with PyClical
========================================
...
```
> git clone git@github.com:penguian/glucat.git glucat-0.98a3
> cd glucat-0.98a3
> make -f admin/Makefile.common bootstrap
```
This results in a directory structure that includes glucat-0.98a3/configure,
allowing you to make and install GluCat in the same way as if you had downloaded
and unzipped the tarball glucat-0.98a3.tar.gz.
...
glucat-0.98a3. Under this directory you should see a number of subdirectories,
including `./admin`, `./doc`, `./gfft_test`, `./glucat`, `./m4`, `./products`,
`./pyclical`, `./squaring`, `./test`, `./test_doctest`, `./test_move`, and `./transforms`.

The following instructions are meant to be used with `glucat-0.98a3` as the
current directory.
...
As briefly described above, the simplest way to install this package is:
 ...
 1. `cd` to the `glucat-0.98a3` directory containing the source code and type
...
produces as many copies of the whole directory `glucat-0.98a3` as there are lines
in `./test/config-options.txt`, naming them `glucat-0.98a3.1` to `glucat-0.98a3.12`,
in the parent directory of `glucat-0.98a3`. This allows the effect of each set
...
GluCat 0.98a3 with PyClical has so far been built and tested using:
